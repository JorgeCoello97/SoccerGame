class GameScreen
{
  ...
  GameScreen(FSM _game){
    ...
    //PLAYER
    player = new Player(width/2, height - (height/4), "player1.png");
    player.x = (width/2) - (player.img.width/2);
    
    
    bttnReturn = new Button(width/2, (height/2)+(height/6), 800, 250, "RETURN");
  }
  ...
  void Draw(){
    ...
    if(started){
      ...
      if(endGame){
          ...
          textSize(170);
          textAlign(CENTER,CENTER);
          int winner = stadium.puntuation1 > stadium.puntuation2 ? 1 : 2;
          text("END GAME\nWINNER \nPLAYER "+winner, width/2, height/2);
          popStyle();
          started = false;
          if(!resultSaved)
          {
...
void oscEvent(OscMessage oscMessage){
  ...
  if(oscMessage.addrPattern().equals("puntuation")){
    ...
    game.gameScreen.scoredByPlayer=1;
  }
